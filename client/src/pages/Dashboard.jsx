...
export default function Dashboard() {
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
      ...
      {/* Hero */}
      <motion.div variants={fadeUp} className="relative rounded-3xl overflow-hidden p-8 sm:p-10" style={{ background: 'linear-gradient(135deg, #1e1040 0%, #0f172a 50%, #0c1a2e 100%)', border: '1px solid rgba(99,102,241,0.2)' }}>
        {/* Decorative blobs */}
        <div className="absolute top-0 right-0 w-80 h-80 bg-indigo-600/20 rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute bottom-0 left-1/3 w-60 h-60 bg-purple-600/15 rounded-full blur-[80px] pointer-events-none" />
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-slate-400 text-base mb-8 max-w-lg">
            Your portfolio is performing remarkably well. Approval rate is up <span className="text-emerald-400 font-semibold">2.1%</span> this week.
          </p>
          <div className="flex items-center gap-3">
            ...
            <Link to="/marketplace" className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-slate-300 font-semibold text-sm hover:text-white hover:bg-white/5 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {stats.map((stat, i) => (
          ...
          >
            <div className="absolute -top-8 -right-8 w-32 h-32 rounded-full blur-2xl opacity-20" style={{ background: `linear-gradient(135deg, ${stat.from}, ${stat.to})` }} />
            <div className="flex justify-between items-start mb-5 relative">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${stat.from}30, ${stat.to}20)`, border: `1px solid ${stat.from}40` }}>
      ...
      <motion.div variants={fadeUp} className="glass-card rounded-2xl p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-base font-semibold text-slate-200">Applications vs Approvals</h2>
            <p className="text-xs text-slate-500 mt-0.5">Performance over the last 7 months</p>
          </div>
          <select
            className="input-dark text-xs py-1.5 px-3 w-auto"
            style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '10px' }}
          >
            <option style={{ background: '#0d1220' }}>Last 6 Months</option>
            <option style={{ background: '#0d1220' }}>This Year</option>
          </select>
        </div>
        <div className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={customTooltipStyle} cursor={{ stroke: 'rgba(99,102,241,0.2)', strokeWidth: 1 }} />
